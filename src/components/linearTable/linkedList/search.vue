<template>
  <div>
    <baseDiv ref="base" :codeDataList="codeDataList" :levelList="levelList" :option="option" :title="title">
      <div slot="animation-div">
        <canvas id="canvas" height="340" width="800"></canvas>
        <span style="position: absolute;top:10px;left:10px;">查找值:{{Player.val}}</span>
      </div>
      ...
      <el-form slot="add-dialog-div">
        <el-form-item label="数组:" label-width="60px">
          <el-input v-model="dataString" placeholder="数组" type="textarea"></el-input>
        </el-form-item>
        <el-form-item label="查找值:" label-width="80px">
          <el-input v-model.number="Player.val"></el-input>
        </el-form-item>
      </el-form>
    </baseDiv>
  </div>
...
export default {
  data() {
    return {
      codeDataList: [
        `//单链表查找
int insertNodeByIndex(int number, Node* head) {
    ...
    return -1;
        ...
        `//单链表查找
int insertNodeByIndex(int number,struct Node* head) {
    ...
    return -1;
        ...
        `//单链表查找
public int insertNodeByIndex(int number, Node head) {
    ...
    moveCursor() {
      ...
      const innerLoop = () => {
        if (that.isPlay && !this.isEnd && !(that.set.has(k))) {
          ...
          if (k === 9 && this.Player.val === this.datalist[this.index - 1]) {
            that.editor.setCursor(10, 0)
            this.isEnd = true
            alert('找到了位置为' + this.index)
          } else if (k === 9 && this.Player.val !== this.datalist[this.index - 1]) {
            k = 11
          }
          if (k === 14 && this.index < this.Player.len) {
            k = 4
            this.index++
          }
          if (k === 16) {
