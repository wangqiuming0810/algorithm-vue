...
<script>
import { MyArray, Rect, drawArrow } from '@/assets/js/myArray'
...
export default {
  data() {
    return {
      codeDataList: [
        `//线性表插入
public void insert(int* arr,int len,int val,int index){
  ...
  arr[index]=val;
        ...
        `//线性表插入
public void insert(vector<int>& a,int len,int val,int index){
  for(int i=len-1;i>index;i--)
  {
...
public void insert(int[] arr,int len,int val,int index){
  ...
  methods: {
    moveCursor() {
      ...
      let i = this.Player.status.i === -1 ? this.myArray.arr.length - 1 : this.Player.status.i
      const innerLoop = () => {
    ...
    drawInsert() {
      ...
      const change = (i) => {
        if (base.isPlay && this.isDraw) {
          drawItems[i + 1].value = drawItems[i].value
          drawItems[i + 1].color = 'green'
      ...
      switch (status.fun) {
        ...
        default:
          ...
          drawItems.push(new Rect(x + drawItems.length * offset, y, offset, boxHight, 'yellow', ''))
          this.myArray.draw(context)
          innerLoop(drawItems.length - 2)
      }
    }
  }
}
