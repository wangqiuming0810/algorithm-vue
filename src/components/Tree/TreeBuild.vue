    ...
    >
      ...
      <el-form slot="change-dialog-div">
        <el-form-item label="结点数:" label-width="80px">
          <el-input v-model.number="Player.num" placeholder="结点树数目"/>
        </el-form-item>
      </el-form>
      <el-form slot="add-dialog-div">
        <el-form-item label="先序串:" label-width="60px">
          <el-input v-model="dataString" placeholder="先序字符串" type="textarea"/>
        </el-form-item>
      </el-form>
    </baseDiv>
  </div>
...
public static TreeNode preOrderBuilder(String s,int[] i){
  ...
  methods: {
    ...
    moveCursor() {
      let k = this.Player.moveCursor.k
      const stack = []
      const that = this.base
      const innerLoop = () => {
    ...
    randomCreateTree() {
      ...
      const {
        root,
        str
      } = Tree.randomCreateTree(n, this.randomString)
      this.dataString = str
      this.treeData = root
      this.refeshString = this.dataString
    },
        ...
        if (base.isPlay && that.isDraw) {
          ...
          if (root.right) {
            const p = {
              ...
              x1: x,
              y1: y,
              x2: x + (s + 2 * radius) * Math.sin(alpha),
              y2: y + (s + 2 * radius) * Math.cos(alpha),
              alpha1: alpha,
              alpha2: a - b,
              n: n + 1,
              s: s
          ...
          if (root.left && that.isDraw) {
            ...
            setTimeout(() => {
              preOrderTraverse(ctx, root.left, x - (s + 2 * radius) * Math.sin(alpha), y + (s + 2 * radius) * Math.cos(alpha), n + 1, a - b)
            }, base.getSpeed())
          } else {
            setTimeout(() => {
              drawRightNode(ctx, root, x, y, n, alpha)
            }, base.getSpeed())
          }
        } else {
          that.Player.status = {
      ...
      function drawRightNode(ctx, root, x, y, n, alpha) {
        if (base.isPlay && that.isDraw) {
          ...
          if (p.root) {
            ...
            setTimeout(() => {
              drawNull(ctx, root, x, y, n, alpha, p)
            }, base.getSpeed())
          }
        } else {
          that.Player.status = {
